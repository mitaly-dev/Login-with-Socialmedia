...
const Infopage = () => {
	...
	const signOutFromAll=()=>{
		...
		navigate('/')
		.then(()=>{
			alert('Sign out form this method')
		})
	}

    return (
		<>
		<button onClick={signOutFromAll} className='font-bold my-5 ml-20 border rounded-lg px-3 text-center py-2 hover:bg-black hover:text-white'>Sign Out</button>
	...
	<div className="flex items-center justify-between p-3">
		<div className="flex items-center space-x-2">
			<img src={photoURL} alt="" className="object-cover object-center w-8 h-8 rounded-full shadow-sm bg-gray-500 border-gray-700" />
			<div className="-space-y-1">
				<h2 className="text-sm font-semibold leading-none">{displayName}</h2>
				<span className="inline-block text-xs leading-none text-gray-400">{email?`Email : ${email}`:undefined}</span>
			</div>
		</div>
